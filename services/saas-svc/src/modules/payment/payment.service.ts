...
export class PaymentService implements OnModuleInit {
  ...
  // 公开 API 地址 = saas-svc 自己暴露的;前端跳回地址 = apps/web
  private apiBase() { return process.env.PUBLIC_API_URL || "http://localhost:4001/api/v1/saas"; }
  ...
  private toGatewayConfig(row: any, returnUrl?: string): GatewayConfig {
    return {
      ...
      // 注:callback URL 走根路径 /payments/callback/:gateway(各家文档约定俗成)
      notifyUrl: `${this.apiBase().replace(/\/api\/v1\/saas$/, "")}/payments/callback/${row.code}`,
  ...
  private async markPaid(paymentNo: string, providerTxnId?: string) {
    ...
    if (r.changed && r.payment.orderId) {
      await this.orders.fulfill(r.payment.orderId, r.payment.gatewayCode);
    }
    return r;
  }
